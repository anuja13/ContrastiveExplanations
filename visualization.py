...
def linear_transform(attributions, clip_above_percentile=99.9, clip_below_percentile=98, low=0.2, plot_distribution=False):
...
def compute_threshold_by_top_percentage(attributions, percentage=70, plot_distribution=True):
...
def overlay_function(attributions, image):
    return np.clip(5*image + attributions, 0, 255)

def visualize(attributions, image, positive_channel=G, negative_channel=R, polarity='positive', \
                clip_above_percentile=99, clip_below_percentile=0, morphological_cleanup=False, \
